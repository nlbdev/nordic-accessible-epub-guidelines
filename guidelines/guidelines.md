...
The EPUB [container file](https://www.w3.org/TR/epub-33/#sec-container-metainf-container.xml) must be given the production number provided with the order and correspond with the identifier stored in the [^dc:identifier^] element located in the [^package^] metadata.
